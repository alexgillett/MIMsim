#' Single pedigree mixed inheritance model simulations of disease (2)
#'
...
#'
#' @return \code{mim_single_ped_countFH} returns either 1. a data frame with 15 columns containing information about the simulated family, or 2. \code{NULL}, meaning there was no individual in Generation III who matched the eligibility criteria specified in \code{eligible}.
#'
...
#'
#' @export
mim_single_ped_countFH <-
function(i, penetrance, maf, h2, r2, eligible = "Genotype", pConsent=c(20,0,0,0), sibship = "Variable", mean_sibship=c(2.25, 2.25, 2.25), offspring = TRUE, gene_drop = TRUE, store = FALSE, maxit = 1E+06){

###########################################	
###### SET UP #######
###########################################
## NOTE: Error messages are in wrapper function 'mimsim'
...
#Mean shift/ Effect of not being a G carrier (G==0)
beta0 <- 0
## this currently only allows risk shift...
#Mean shift/effect of having G == 1
beta1 <- D_threshold - qnorm(1-penetrance[1,2], mean = 0, sd=sqrt(varL_G))
#Mean shift/effect of having G == 2
beta2 <- D_threshold - qnorm(1-penetrance[1,3], mean = 0, sd=sqrt(varL_G))
...
#Assign E
gen1_mates[,10] <- rnorm(n=dim(gen1_mates)[1], mean=0, sd=sqrt(r2))
# Assign Sex. Col 7.
...
#	FH_GP <- as.numeric(rep(mapply(function(tmp_MF, tmp_GP, z){tmp_GP[tmp_GP[,2] == tmp_MF[z,1],12] + tmp_GP[tmp_GP[,2] == tmp_MF[z,2],12] + tmp_GP[tmp_GP[,2] == tmp_MF[z,3],12] + tmp_GP[tmp_GP[,2] == tmp_MF[z,4],12]}, z=1:dim(Father_d)[1], tmp_MF=list(cbind(Father_d[,4:5], Mother_d[,4:5])), tmp_GP=list(rbind(pedigree_i[1:2, ], gen1_mates))), Father_d[,13]) > 0)
	...
	eligible_tmp$FH_S <- eligible_tmp$FH_S - 1
	
	FH_consent <- exp(matrix(cbind(1, eligible_tmp$FH_P, eligible_tmp$FH_S, eligible_tmp$FH_GP), nrow=dim(eligible_tmp)[1])%*%matrix(pConsent))/(1+exp(matrix(cbind(1, eligible_tmp$FH_P, eligible_tmp$FH_S, eligible_tmp$FH_GP), nrow=dim(eligible_tmp)[1])%*%matrix(pConsent)))	
	
	Consent <- apply(matrix(FH_consent), 1, function(x) rbinom(1,1,x))
...
#				setkey(av_missing, Sex, ID)
		...
		if(offspring){
			parent_1[parent_1[,2] ==pb_ID, 13] <- rpois(1, mean_sibship[3])
